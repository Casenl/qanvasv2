...
export function AlignmentToolbar({
  ...
  const hasStylable = selectedItems.some((item) =>
    ["shape", "text", "sticky-note", "frame", "pen", "line", "arrow"].includes(
      item.entityType,
    ),
  );

  // Get representative item for current values
  const firstItem = selectedItems.find((item) =>
    ["shape", "text", "sticky-note", "frame", "pen", "line", "arrow"].includes(
      item.entityType,
    ),
  );
  ...
  if (firstItem) {
    if (firstItem.entityType === "shape") {
      ...
      currentStrokeColor =
        firstItem.data?.style?.strokeColor || currentStrokeColor;
      ...
      if (firstItem.data?.label) {
        ...
        currentFillColor =
          firstItem.data?.labelBackgroundColor || "rgba(255, 255, 255, 0.9)";
        currentFontFamily =
          firstItem.data?.labelFontFamily || "Titillium Web, sans-serif";
        currentFontWeight = firstItem.data?.labelBold ? "bold" : "normal";
        currentFontStyle = firstItem.data?.labelItalic ? "italic" : "normal";
      }
    } else if (
      firstItem.entityType === "line" ||
      firstItem.entityType === "arrow"
    ) {
  ...
  return (
    <div
      className={
        position
          ? "absolute z-50 flex items-center gap-1 p-2 rounded-xl shadow-2xl border backdrop-blur-xl animate-in fade-in zoom-in-95 duration-200"
          : "fixed bottom-20 left-1/2 -translate-x-1/2 z-40 flex items-center gap-1 p-2 rounded-xl shadow-2xl border backdrop-blur-xl"
      }
      style={{
    ...
    >
      ...
      {hasStylable && onStyleChange && (
        <>
          ...
          {(selectedItems.some((item) =>
            ["shape", "text", "sticky-note", "frame"].includes(item.entityType),
          ) ||
            selectedItems.some(
              (item) =>
                ["pen", "line", "arrow"].includes(item.entityType) &&
                item.data?.label,
            )) && (
            <div
          ...
          {selectedItems.some((item) =>
            ...
            <>
              <TextStyleControls
                ...
                onUnderlineToggle={() =>
                  onStyleChange("underline", !currentUnderline)
                }
                onStrikethroughToggle={() =>
                  onStyleChange("strikethrough", !currentStrikethrough)
                }
              />
              ...
              {/* Text Alignment */}
              <div
                className="flex bg-gray-100 rounded-lg p-0.5 mx-1"
      ...
      {selectedCount === 1 &&
        selectedItems[0] &&
        onStyleChange &&
        ["pen", "line", "arrow"].includes(selectedItems[0].entityType) && (
          <>
            <button
              onClick={() => {
              ...
              title={
                selectedItems[0].data?.label
                  ? "Label exists (double-click to edit)"
                  : "Add Label"
              }
            >
            ...
            {selectedItems[0].data?.label && (
              <LabelStyleControls
                ...
                onLabelColorChange={(color) =>
                  onStyleChange("labelColor", color)
                }
                onLabelBackgroundChange={(color) =>
                  onStyleChange("labelBackgroundColor", color)
                }
                onLabelSizeChange={(size) => onStyleChange("labelSize", size)}
                onLabelFontFamilyChange={(font) =>
                  onStyleChange("labelFontFamily", font)
                }
                onLabelBoldToggle={() =>
                  onStyleChange("labelBold", currentFontWeight !== "bold")
                }
                onLabelItalicToggle={() =>
                  onStyleChange("labelItalic", currentFontStyle !== "italic")
                }
              />
            )}
      ...
      {selectedCount >= 2 && onCreateSolution && (
        <>
          ...
          <button
            ...
            style={{
              color:
                needsMultiple || !onCreateSolution
                  ? "var(--color-text-muted)"
                  : "var(--color-text)",
              backgroundColor: "transparent",
            }}
