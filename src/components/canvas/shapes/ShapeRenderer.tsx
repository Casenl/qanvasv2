...
import { ShapeData } from '@/lib/types/shapeTypes';
 ...
 */
export function ShapeRenderer({ data, isSelected = false, onClick, onUpdate }: ShapeRendererProps) {
    ...
    const { type, width, height, style = {} } = data;
    const {
        fillColor = '#3b82f6',
        strokeColor = '#1e40af',
        strokeWidth = 2,
            ...
            >
                {isEditingText ? (
                    ...
                    <div
                        ...
                        dangerouslySetInnerHTML={{ __html: data.text || (onUpdate && isSelected ? '<span style="opacity:0.5; font-style:italic">Type to add text</span>' : '') }}
                    />
                )}
            </div>
        </div>
