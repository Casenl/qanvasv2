...
export function useDrawingMode(config: DrawingModeConfig) {
  ...
  const createShapeItem = useCallback(
    (x: number, y: number, shapeType: string): CanvasItem => {
   ...
   */
  const createStickyNoteItem = useCallback(
    (x: number, y: number): CanvasItem => {
   ...
   */
  const createPathItem = useCallback(
    (x: number, y: number, points: { x: number; y: number }[]): CanvasItem => {
   ...
   * Create a frame item
   */
  const createFrameItem = useCallback(
    (
      x: number,
      y: number,
      width: number = 400,
    ...
    (e: React.MouseEvent<HTMLDivElement>): boolean => {
      ...
      switch (activeTool) {
        ...
        case "heart":
          ...
          setDebugInfo?.(
            `Created ${activeTool} at (${Math.round(x)}, ${Math.round(y)})`,
          );
          break;
        ...
        case "text":
          ...
          setDebugInfo?.(
            `Created text at (${Math.round(x)}, ${Math.round(y)})`,
          );
          break;
        ...
        case "sticky-note":
          ...
          setDebugInfo?.(
            `Created sticky note at (${Math.round(x)}, ${Math.round(y)})`,
          );
          break;
            ...
            if (file) {
              ...
              reader.onload = (event) => {
                ...
                img.onload = () => {
                  ...
                  setDebugInfo?.(
                    `Created image at (${Math.round(x)}, ${Math.round(y)})`,
                  );
                  onToolReset?.();
                };
                img.src = imageUrl;
  ...
  const handleDrawEnd = useCallback(() => {
    ...
    if (
      ...
      // Create path item
      newItem = createPathItem(
        drawingState.startX,
        drawingState.startY,
        drawingState.points,
      );
      setDebugInfo?.(
        `Created pen path with ${drawingState.points.length} points`,
      );
    } else if (
      (activeTool === "line" || activeTool === "arrow") &&
      drawingState.endX !== undefined &&
      drawingState.endY !== undefined
    ) {
