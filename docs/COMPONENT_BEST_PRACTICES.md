...
```tsx
/**
 * ProductCard - Displays a product with name, price, and actions
 * 
 * @component
 * @example
 * ```tsx
...
```tsx
/**
 * useSnapshotManager - Manages canvas snapshots (create, load, delete, compare)
 * 
 * @returns {Object} Snapshot manager interface
 * @returns {Snapshot[]} snapshots - Array of saved snapshots
 * @returns {Function} createSnapshot - Create a new snapshot
 * @returns {Function} loadSnapshot - Load a snapshot by ID
 * 
 * @example
 * ```tsx
 * const { snapshots, createSnapshot, loadSnapshot } = useSnapshotManager();
 * 
 * // Create snapshot
 * createSnapshot('Phase 1', canvasData);
 * 
 * // Load snapshot
 * const data = loadSnapshot('snapshot-123');
 * ```
 */
...
### 1. State Updates with Side Effects

```tsx
// ❌ Bad: Side effect in state update
setItems(prev => prev.map(item => {
  if (item.id === activeId) {
    const newItem = { ...item, id: generateId() };
    setTimeout(() => {
      setItems(current => [...current, newItem]); // Side effect!
    }, 0);
    return item;
  }
  return item;
...
const isCopying = checkModifierKeys();

if (isCopying) {
  const newItemId = generateId();
  setItems(prev => {
    const original = prev.find(it => it.id === activeId);
    if (!original) return prev;
    return [...prev, { ...original, id: newItemId }];
  });
  selectItem(newItemId); // Side effects AFTER state update
} else {
  setItems(prev => prev.map(it => 
    it.id === activeId ? { ...it, x: newX, y: newY } : it
  ));
}
```
...
function ProductList() {
  const { products, loading } = useProducts(); // Hook handles data
...
<GenericDataTable
  ...
  renderCell={(row, col) => renderProductCell(row, col)}
  onSort={(col) => handleProductSort(col)}
...
**Remember**: Don't abstract until you have 3+ similar use cases.
...
- [ ] Are all hooks at the top, in consistent order?
...
- [ ] Are there any repeated patterns that could be extracted?
- [ ] Is state management clean and predictable?
- [ ] Are there any performance concerns (unnecessary re-renders)?
