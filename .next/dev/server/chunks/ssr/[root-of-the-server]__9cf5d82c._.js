...
;
;
function CanvasWorkspace({ canvasRef, items, selectedIds, debugInfo, vendors, multiSelect, onClearItems, onContextMenu }) {
...
function useKeyboardShortcuts(shortcuts, enabled = true) {
    const handleKeyDown = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useCallback"])((event)=>{
        ...
        const isModifier = ctrlKey || metaKey; // Support both Ctrl (Windows) and Cmd (Mac)
        // Ctrl/Cmd + A: Select All
        if (isModifier && key === 'a' && shortcuts.onSelectAll) {
            event.preventDefault();
            shortcuts.onSelectAll();
            return;
        }
        // Ctrl/Cmd + C: Copy
        if (isModifier && key === 'c' && shortcuts.onCopy) {
            event.preventDefault();
            shortcuts.onCopy();
            return;
        }
        // Ctrl/Cmd + V: Paste
        if (isModifier && key === 'v' && shortcuts.onPaste) {
            event.preventDefault();
            shortcuts.onPaste();
            return;
        }
        ...
        // Ctrl/Cmd + Z: Undo
        if (isModifier && key === 'z' && !shiftKey && shortcuts.onUndo) {
            event.preventDefault();
            shortcuts.onUndo();
            return;
        }
        ...
        if (isModifier && key === 'd' && shortcuts.onDuplicate) {
            event.preventDefault();
        ...
        // Arrow keys: Nudge (only if not in input field)
        const target = event.target;
        const isInputField = target.tagName === 'INPUT' || target.tagName === 'TEXTAREA';
        if (!isInputField) {
            const nudgeAmount = shiftKey ? 10 : 1; // Shift = larger nudge
            if (key === 'ArrowUp' && shortcuts.onNudgeUp) {
                event.preventDefault();
                for(let i = 0; i < nudgeAmount; i++)shortcuts.onNudgeUp();
                return;
            }
            if (key === 'ArrowDown' && shortcuts.onNudgeDown) {
                event.preventDefault();
                for(let i = 0; i < nudgeAmount; i++)shortcuts.onNudgeDown();
                return;
            }
            if (key === 'ArrowLeft' && shortcuts.onNudgeLeft) {
                event.preventDefault();
                for(let i = 0; i < nudgeAmount; i++)shortcuts.onNudgeLeft();
                return;
            }
            if (key === 'ArrowRight' && shortcuts.onNudgeRight) {
                event.preventDefault();
                for(let i = 0; i < nudgeAmount; i++)shortcuts.onNudgeRight();
                return;
            }
        }
    }, [
...
"[project]/src/components/canvas/board/CanvasBoard.tsx [app-ssr] (ecmascript)", ((__turbopack_context__, module, exports) => {

const e = new Error("Could not parse module '[project]/src/components/canvas/board/CanvasBoard.tsx'\n\nExpected ',', got 'return'");
e.code = 'MODULE_UNPARSABLE';
throw e;
}),
];

//# sourceMappingURL=%5Broot-of-the-server%5D__9cf5d82c._.js.map